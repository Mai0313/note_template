...
以 [Zensical](https://zensical.org/) + GitHub Pages 为核心的**笔记仓库 template**。把 markdown 丢进 `docs/`、push 到 `main`,剩下的事情全部自动完成:linting、发布到 GitHub Pages、另外 build 一个静态站点 Docker image 用于离线备份或自托管。
...
- **静态站点 Docker image** — `build_image.yml` 会 build 一个 `ghcr.io/<owner>/<repo>:latest` (nginx + 已 build 好的 site),方便自托管或备份。
- **完整 CI 卫生** — semantic PR 标题、auto-labeler、release-drafter、dependabot auto-merge、secret/CodeQL/Trivy 扫描、pre-commit 每日自动更新。
- **内建 Devcontainer** — 用 VS Code 打开即有 zsh + oh-my-zsh + uv 的编辑环境。
...
每次 push `main` 会 push 一个 nginx image (已内置渲染好的 site):
...
Host 端口直接写在 `docker-compose.yaml` 里,若 `8080` 撞到就改那边。
...
- 其他不需要配置 — `GITHUB_TOKEN` 已足以推 GHCR。
...
├── docker/Dockerfile          # 两阶段:Zensical build → nginx
├── docs/
│   └── index.md               # 首页 (其他子目录你自己建)
├── docker-compose.yaml        # 本机 host 笔记站
