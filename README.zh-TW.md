...
以 [Zensical](https://zensical.org/) + GitHub Pages 為核心的**筆記專案 template**。把 markdown 丟進 `docs/`、push 到 `main`,剩下的事情都會自動完成:linting、發佈到 GitHub Pages、另外 build 一個靜態網站 Docker image 方便離線備份或自架。
...
- **靜態網站 Docker image** — `build_image.yml` 會 build 一個 `ghcr.io/<owner>/<repo>:latest` (nginx + 已 build 好的 site),方便自架或備份。
- **完整 CI 衛生** — semantic PR 標題、auto-labeler、release-drafter、dependabot auto-merge、secret/CodeQL/Trivy 掃描、pre-commit 每日自動更新。
- **內建 Devcontainer** — 用 VS Code 打開即有 zsh + oh-my-zsh + uv 的編輯環境。
...
3. **改名字**:
    ...
    - 更新 `docker/Dockerfile` 和 `.devcontainer/Dockerfile` 的 label。
4. **確認**:
    ```bash
...
每次 push `main` 會 push 一個 nginx image (已內建渲染好的 site):
...
Host port 直接寫在 `docker-compose.yaml` 裡面,若 `8080` 撞到就改那邊即可。
...
| `build_image.yml`           | push `main` / tag `v*` | Build nginx image + push 到 GHCR      |
...
- 其他不需要設定 — `GITHUB_TOKEN` 就足夠推 GHCR。
...
├── docker/Dockerfile          # 兩階段:Zensical build → nginx
├── docs/
│   └── index.md               # 首頁 (其他子資料夾你自己建)
├── docker-compose.yaml        # 本機 host 筆記站
